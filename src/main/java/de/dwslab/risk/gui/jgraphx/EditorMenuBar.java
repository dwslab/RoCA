...
public class EditorMenuBar extends JMenuBar {
    ...
    public EditorMenuBar(final RoCA editor) {
        ...
        menu.add(editor.bind("Floyd-Roy-Warshall", new AnalyzeGraph(AnalyzeType.FLOYD_ROY_WARSHALL,
                aGraph)));
        menu.add(editor.bind("Get Components", new AnalyzeGraph(AnalyzeType.COMPONENTS, aGraph)));
        menu.add(editor
                .bind("Make Connected", new AnalyzeGraph(AnalyzeType.MAKE_CONNECTED, aGraph)));
        ...
        menu.add(editor.bind("Get cut vertices", new AnalyzeGraph(AnalyzeType.GET_CUT_VERTEXES,
                aGraph)));
        ...
        menu.add(editor
                .bind("Is biconnected", new AnalyzeGraph(AnalyzeType.IS_BICONNECTED, aGraph)));
    ...
    public static class InsertGraph extends AbstractAction {
        ...
        public void actionPerformed(ActionEvent e) {
            if (e.getSource() instanceof mxGraphComponent) {
                ...
                dialog.setLocation(screenSize.width / 2 - (frameSize.width / 2), screenSize.height
        ...
        public void actionPerformed(ActionEvent e) {
            if (e.getSource() instanceof mxGraphComponent) {
                ...
                if (analyzeType == AnalyzeType.IS_CONNECTED) {
                    ...
                    for (Object cutVertice : cutVertices) {
                        System.out.print(" " + model.getValue(cutVertice));
                    }
                    ...
                    for (Object cutEdge : cutEdges) {
                        System.out.print(" "
                                + Integer.parseInt((String) model.getValue(aGraph.getTerminal(
                                        cutEdge, true)))
                                + "-"
                                + Integer.parseInt((String) model.getValue(aGraph.getTerminal(
                                        cutEdge, false))));
                    ...
                    try {
                        ...
                        for (Object sourceVertice : sourceVertices) {
                            System.out.print(" " + model.getValue(sourceVertice));
                        }
                    ...
                    try {
                        ...
                        for (Object sinkVertice : sinkVertices) {
                            System.out.print(" " + model.getValue(sinkVertice));
                        }
                    ...
                    try {
                        ...
                        for (int i = 0; i < vertexNum; i++) {
                            ...
                            for (int j = 0; j < vertexNum; j++) {
                                System.out.print(" " + Math.round((Double) dist[i][j] * 100.0)
                                        / 100.0);
                            }
                        ...
                        for (int i = 0; i < vertexNum; i++) {
                            ...
                            for (int j = 0; j < vertexNum; j++) {
                                if (paths[i][j] != null) {
                                    System.out.print(" "
                                            + costFunction.getCost(view.getState(paths[i][j])));
                                } else {
                                    System.out.print(" -");
                                }
                            }
                        ...
                        try {
                            ...
                            System.out
                                    .print("The path from "
                                            + costFunction.getCost(view.getState(vertices[0]))
                                            + " to "
                                            + costFunction.getCost((view
                                                    .getState(vertices[vertexNum - 1]))) + " is:");

                            for (Object element : path) {
                                System.out
                                        .print(" " + costFunction.getCost(view.getState(element)));
                            }
